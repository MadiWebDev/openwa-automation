...
export async function POST(request: Request) {
  try {
    ...
    const response = await openwa(`/api/sessions/${encodeURIComponent(body.sessionId)}/webhooks`, { method: 'POST', body: JSON.stringify({ url, secret, events: body.events || ['message.sent', 'message.ack', 'message.failed', 'session.status'], retryCount: body.retryCount ?? 3 }) })
